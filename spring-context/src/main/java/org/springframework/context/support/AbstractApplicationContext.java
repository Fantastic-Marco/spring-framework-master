...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 方法内主要对已有的Bean进行摧毁，关闭BeanFactory，并创建新的BeanFactory。
			//然后解析配置文件，形成BeanDefinition对象，存储起来。
			...
			try {
				...
				/**
				 * 程序执行到当前方法时已经执行了所有的BeanFactory后置处理器，并且加载了一些特殊的bean。
				 * 这个方法主要会初始化剩下的没有设置成懒加载模式的所有单例Bean。
				 */
	 ...
	 * 配置工厂的标准上下文特征，例如上下文的类加载器以及后继处理器
	 * @param beanFactory the BeanFactory to configure
	 */
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		// Tell the internal bean factory to use the context's class loader etc.
		// 告诉内部的bean工厂使用上下文的类加载器等等
		...
		// 在普通工厂中Bean工厂接口不需要被注册为可解析类型
		// 消息源注册为一个 bean，并且用于自动装配
	 ...
	 * 判断当前上下文的Bean工厂是否处于活跃状态
	 * throwing an {@link IllegalStateException} if it isn't.
	 * <p>Invoked by all {@link BeanFactory} delegation methods that depend
	 * on an active context, i.e. in particular all bean accessor methods.
	 * 被所有依赖活跃上下文的Bean工厂委派方法调用，特别是所有bean的访问器方法
	 * <p>The default implementation checks the {@link #isActive() 'active'} status
	 * of this context overall. May be overridden for more specific checks, or for a
	 * no-op if {@link #getBeanFactory()} itself throws an exception in such a case.
	 * 默认实现会检查整个上下文的 活跃 状态。可能会覆盖以进行更详细的检查，或者在某些情况下 getBeanFactory（）
	 * 方法本身会抛出异常
	 */
