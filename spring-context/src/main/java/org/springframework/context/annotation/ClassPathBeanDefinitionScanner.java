 ...
 * 一个用于在classpath中检测候选人的bean定义扫描器
 * 通过给定的注册表（bean工厂或者ApplicationContext）注册相关的bean定义
 * <p>Candidate classes are detected through configurable type filters. The
 * default filters include classes that are annotated with Spring's
 * 候选类都是通过可配置类型过滤器被检测，默认的过滤器包括被spring注解申明的类，例如：以下几种
...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	public int scan(String... basePackages) {
		...
		logger.debug(String.format("registry %s has bean definition count %d",this.registry,beanCountAtScanStart ));
		//执行扫描
		doScan(basePackages);
